...
class Payroll extends CI_Controller {
	...
	public function data_list()
	{
		...
		foreach ($list as $datanya) {
			 $tgl_transfer = date('d M Y ', strtotime($datanya->tgl_input)); 
            ...
            $row[] = "<span class='text-success'>".rupiah($datanya->jumlah_total)."</span>";
              if ($datanya->status == "selesai"){
                $row[] = "<span class='text-success'> Sudah ditransfer </span>";
                $row[] = htmlentities($tgl_transfer);
                $row[] = '
                    <a type="button" class="btn btn-primary btn-sm" onclick="selesai('."'".$datanya->id_kehadiran_lembaga."'".')""
        			title="Track" ><i class="fas fa-eye mr-1" ></i> Lihat Rincian</a>';
              } else {
  ...
  public function get_byid($id)
	{
		...
		foreach ($list as $sow) {
			$html_item .= '<tr>';
				...
				$html_item .= '<td><h6>'.$sow->nama_bank.'</h6> <input type="hidden" name="id_kehadiran[]" value="'.$sow->id_kehadiran.'" ></td>';
