...
class CustomPDF extends FPDF {
    function Header() {
        if ($this->PageNo() > 1) {
            ...
            $x_group = $this->GetX(); // Save X start of groups (10+8+90 = 108)
            $this->Cell(45,5,'Kehadiran 1 (Siang)',1,0,'C');
            $this->Cell(60,5,'Kehadiran 2 (Shift)',1,0,'C');
            $this->Cell(45,5,'Kehadiran 3 (Dinihari)',1,0,'C');
...
// KOP SURAT (Sama seperti Kehadiran_struktural/Cetak)
...
$pdf->Cell(0, 5, 'RINCIAN BAROKAH SATPAM', 0, 1, 'C'); // Changed Title
$pdf->SetFont('arial', '', 11);
$pdf->Cell(0, 5, 'Bulan : '.$periode->bulan.' '.$periode->tahun, 0, 1, 'C');

$pdf->ln(5);

// HEADER TABEL (Manual for Page 1)
...
foreach($isilist as $row) {
    $nominal_transport = (int)$row->nominal_transport;
    
    $jumlah_transport = ((int)$row->jumlah_hari) * $nominal_transport;
    $jumlah_barokah   = ((int)$row->jumlah_shift) * $rank;
    $jumlah_konsumsi  = ((int)$row->jumlah_dinihari) * $konsumsi;
    
    $isDanru = ($row->is_danru == '1');
    $nominalDanruRow = $isDanru ? $honorDanru : 0;
    
    $diterima = $jumlah_transport + $jumlah_barokah + $jumlah_konsumsi + $nominalDanruRow;
    
    $grand_transport += $jumlah_transport;
    $grand_barokah   += $jumlah_barokah;
    $grand_dinihari  += $jumlah_konsumsi;
    $grand_danru     += $nominalDanruRow;
    $grand_total     += $diterima;

    $pdf->Cell(8,7,$no++,1,0,'C');
    $pdf->Cell(90,7, $row->gelar_depan.' '.$row->nama_lengkap.' '.$row->gelar_belakang,1,0,'L'); // Width 60 -> 90
    
    // Kehadiran 1
    $pdf->Cell(10,7,$row->jumlah_hari,1,0,'C');
    $pdf->Cell(15,7,number_format($nominal_transport,0,',','.'),1,0,'R');
    $pdf->Cell(20,7,number_format($jumlah_transport,0,',','.'),1,0,'R');
    
    // Kehadiran 2
    $pdf->Cell(10,7,$row->jumlah_shift,1,0,'C');
    $pdf->Cell(20,7,number_format($rank,0,',','.'),1,0,'R');
    $pdf->Cell(30,7,number_format($jumlah_barokah,0,',','.'),1,0,'R');
    
    // Kehadiran 3
    $pdf->Cell(10,7,$row->jumlah_dinihari,1,0,'C');
    $pdf->Cell(15,7,number_format($konsumsi,0,',','.'),1,0,'R');
    $pdf->Cell(20,7,number_format($jumlah_konsumsi,0,',','.'),1,0,'R');
    
    // Danru
    $pdf->Cell(35,7,number_format($nominalDanruRow,0,',','.'),1,0,'R'); // Width 25 -> 35
    
    // Total
    $pdf->SetFont('arial','B',9);
    $pdf->Cell(40,7,number_format($diterima,0,',','.'),1,1,'R'); // Width 30 -> 40
    $pdf->SetFont('arial','',9);
}
...
// Danru
$pdf->Cell(35,7,number_format($grand_danru,0,',','.'),1,0,'R'); // Width 35

// Grand Total
$pdf->Cell(40,7,number_format($grand_total,0,',','.'),1,1,'R'); // Width 40
