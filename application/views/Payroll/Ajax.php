<script src="<?php echo base_url() ?>assets/vendor/global/global.min.js"></script>
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                res.forEach((log, i) => {
                    ...
                    // Tombol resend untuk semua status, warna berbeda
                    let actionBtn = '';
                    if (log.status === 'success') {
                        ...
                        actionBtn = `<button class="btn btn-sm btn-warning" 
                                        onclick="resendWA(${log.id_wa_log}, ${id_lembaga}, '${bulan}', ${tahun})">
                                    <i class="fas fa-redo me-1"></i>Ulang
                                </button>`;
                    } else { // pending
                        actionBtn = `<button class="btn btn-sm btn-secondary" 
                                        onclick="resendWA(${log.id_wa_log}, ${id_lembaga}, '${bulan}', ${tahun})">
                                    <i class="fas fa-redo me-1"></i>Ulang
                                </button>`;
                    }
                ...
                // Show/hide bulk resend button based on failed count
                if (failedCount > 0) {
                    $('#bulkResendBtn').show().find('.failed-count').text(failedCount);
                    $('#bulkResendBtn').data('id_lembaga', id_lembaga);
                    $('#bulkResendBtn').data('bulan', bulan);
                    $('#bulkResendBtn').data('tahun', tahun);
                } else {
                    $('#bulkResendBtn').hide();
                }
            },
    ...
    function bulkResendFailed() {
        const btn = $('#bulkResendBtn');
        const id_lembaga = btn.data('id_lembaga');
        const bulan = btn.data('bulan');
        const tahun = btn.data('tahun');
        const failedCount = btn.find('.failed-count').text();

        Swal.fire({
            title: 'Resend Semua Failed?',
            html: `Akan mengirim ulang <b>${failedCount} pesan</b> yang gagal.<br>Lanjutkan?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Ya, kirim ulang semua!',
            cancelButtonText: 'Batal'
        }).then((result) => {
            if (result.isConfirmed) {
                Swal.fire({
                    title: 'Mengirim ulang...',
                    html: 'Mohon tunggu, sedang memproses <b>' + failedCount + '</b> pesan...',
                    allowOutsideClick: false,
                    didOpen: () => Swal.showLoading()
                });

                $.ajax({
                    url: '<?= site_url('Payroll/bulk_resend_failed') ?>',
                    type: 'POST',
                    data: { 
                            ...
                            Swal.fire({
                                ...
                                html: `
                                    <div class="text-start">
                                        <p><strong>Hasil Resend:</strong></p>
                                        <ul>
                                            <li><i class="fa fa-check text-success"></i> Berhasil: <b>${res.success_count}</b></li>
                                            <li><i class="fa fa-times text-danger"></i> Gagal: <b>${res.failed_count}</b></li>
                                            <li>Total diproses: <b>${res.total}</b></li>
                                        </ul>
                                    </div>
                                `,
                                ...
                                // Refresh modal untuk update status
                                showWaLog(id_lembaga, bulan, tahun);
                            });
                        } else {
                            Swal.fire({
    ...
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
        ...
        <div class="d-flex align-items-center gap-2">
          <button type="button" class="btn btn-warning btn-sm" id="bulkResendBtn" onclick="bulkResendFailed()" style="display:none;">
            <i class="fas fa-redo-alt me-1"></i>Resend Semua Failed (<span class="failed-count">0</span>)
          </button>
