...
Jazakumullah Khairan';
	...
	//untuk testing pengiriman wa, bisa merubah nomor hp ke 1 nomor saja
	$nomor_hp = "081249057246";
    $logData = [
        'id_total_barokah' => $isipesan['id_total_barokah'] ?? null, //ini adalah kolom untuk isian id_total_barokah pengajar, struktural, dan satpam
	...
	if ($result === false) {
        $result = json_encode(['status' => false, 'message' => curl_error($curl)]);
    }
    curl_close($curl);

    // 3️⃣ Update log status berdasarkan response
    $status = 'failed';
