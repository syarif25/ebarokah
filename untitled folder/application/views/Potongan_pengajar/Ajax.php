...
<script>
    ...
    $(function () {
        ...
        table =   $('#tabel_view').DataTable({
            "ajax": {
                "url": "<?php echo site_url('potongan_pengajar/data_list')?>",
                "type": "POST"
            },
            ...
            "ajax": {
                "url": "<?php echo site_url('potongan_pengajar/data_perumana')?>",
                "type": "POST"
            },
    ...
    $(document).ready(function() {
        $('#select_nik').on('change', function() {
            ...
            if (nik != '') {
            $.ajax({
                url: '<?php echo base_url(); ?>Potongan_pengajar/get_penempatan',
                type: 'post',
                data: {nik: nik},
                dataType: 'json',
                ...
                $.each(response,function(index,data){
                    $('#id_penempatan').append('<option value="'+data.id_pengajar+'">'+data.nama_lembaga+ '</option>');
                });
                }
            });
            }
        ...
        $('#jenis_potongan').change(function(){
            var id=$(this).val();
            $.ajax({
                url : "<?php echo base_url('potongan_pengajar/get_potongan')?>/" + id,
                method : "POST",
                data : {id: id},
                async : false,
                dataType : 'json',
     ...
     function edit_potongan(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('potongan_pengajar/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id_potongan_pengajar);
                $('#nama').html(data.nama_lengkap);
                $('#lembaga').html(data.nama_lembaga);
                $('[name="id_penempatan2"]').val(data.id_pengajar);
    ...
    table_by_id =   $('#tabel_view_rincian_umana').DataTable({
            "ajax": {
                "url": "<?php echo site_url('potongan_pengajar/data_rincian_perumana/')?>" + id,
                "type": "POST",
    ...
    function save()
    {
        ...
        var url;
        if(save_method == 'add') {
            url = "<?php echo site_url('Potongan_pengajar/ajax_add')?>";
            var pesan = ' Menambah data';
        } else {
            url = "<?php echo site_url('Potongan_pengajar/ajax_update')?>";
            var pesan = ' Edit data';
        }
                                        ...
                                        <div class="input-group mb-3 input-success-o">
                                            <select name="nama" id="select_nik" onblur="validate()" class="form-control select2">
                                                ...
                                                <?php if($this->session->userdata('jabatan') == 'AdminLembaga'){
                                                    $data = $this->db->query("SELECT * FROM umana, pengajar, lembaga WHERE umana.nik = pengajar.nik AND pengajar.id_lembaga = lembaga.id_lembaga AND lembaga.id_lembaga = '" . $this->session->userdata('lembaga') . "'")->result();
                                                } else {
                                        ...
                                        <div class="input-group mb-3 input-success-o">
                                            <select name="jenis_potongan" id="jenis_potongan"  class="form-control select2">
                                                ...
                                                <?php $data = $this->db->get('potongan')->result();
                                                    foreach($data as $umana){
                                                ?>
                                                <option value="<?php echo $umana->id_potongan; ?>"><?php echo $umana->nama_potongan.' ~ '.rupiah($umana->nominal); ?></option>
                                            <?php } ?>
                                            <option value=""></option>
                                        </select>
                                        </div>
