...
<div class="container-fluid my-3">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
      <button class="btn btn-warning" onclick="window.print()"><i class="fa fa-print mr-1"></i> Cetak</button>
  ...
  <!-- Ringkasan di atas -->
  <div class="mb-3">
    <span class="badge badge-secondary badge-total">Total Transport: <span id="sumTransport"><?= number_format($total_transport??0,0,',','.') ?></span></span>
    <span class="badge badge-secondary badge-total">Total Barokah: <span id="sumBarokah"><?= number_format($total_jumlah_barokah??0,0,',','.') ?></span></span>
    <span class="badge badge-secondary badge-total">Total Dinihari: <span id="sumDini"><?= number_format($total_jumlah_konsumsi_dinihari??0,0,',','.') ?></span></span>
    <span class="badge badge-primary badge-total">Grand Total: <span id="sumGrand"><?= number_format($total_diterima??0,0,',','.') ?></span></span>
  </div>
      ...
      <table id="tabel_validasi" class="table table-bordered table-striped" style="width:100%">
        ...
        <?php
          ...
          foreach ($isilist as $d):
            ...
            $diterima         = $jumlah_transport + $jumlah_barokah + $jumlah_konsumsi;
...
$(function(){
  ...
  $('#tabel_validasi').DataTable({
    ...
    dom:'Bfrtip', buttons:[{extend:'print', text:'Print'}]
  });
  ...
  // submit massal "Setujui"
    ...
    const fd = new FormData(form); // semua hidden input per baris ikut terkirim
    ...
    .done(function (res) {
        ...
        Swal.fire({
            icon: 'success',
            title: 'Berhasil!',
            text: res.message || 'Data barokah satpam berhasil disetujui.',
            timer: 1800,
            showConfirmButton: false
        }).then(() => {
            window.location.href = "<?= site_url('validasi') ?>";
        });
        } else {
